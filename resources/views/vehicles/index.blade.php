...
<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:1rem;margin-bottom:1.5rem;">
    <div class="stat-card">
        <div class="stat-icon" style="background:#eff6ff;">
            <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path d="M3 17h2l1-3h12l1 3h2" stroke="#3b82f6" stroke-width="1.8" stroke-linecap="round"/><circle cx="7.5" cy="18.5" r="1.5" stroke="#3b82f6" stroke-width="1.5"/><circle cx="16.5" cy="18.5" r="1.5" stroke="#3b82f6" stroke-width="1.5"/><path d="M6.5 9l1-3h9l1 3" stroke="#3b82f6" stroke-width="1.5" fill="none"/></svg>
        </div>
    ...
    <div class="stat-card">
        ...
        <div><div class="stat-val" style="color:#d97706;">{{ $stats['maintenance'] }}</div><div class="stat-lbl">En maintenance</div></div>
    </div>
    <div class="stat-card">
    ...
    <div style="padding:.85rem 1.25rem;border-bottom:1px solid #f1f5f9;background:#fafafa;">
        <form method="GET" class="filters-bar">
            ...
            <label style="display:flex;align-items:center;gap:.4rem;font-size:.82rem;color:#64748b;cursor:pointer;">
                <input type="checkbox" name="avec_archives" value="1" @checked(request()->boolean('avec_archives'))> Archives
            </label>
            ...
            @if(request()->anyFilled(['q','status','type','fuel','avec_archives']))
        ...
        <table>
            ...
            <tbody>
                ...
                @php
                    ...
                    $typeMap = [
                        'sedan'=>'Berline','suv'=>'SUV','van'=>'Van','pickup'=>'Pick-up',
                        'truck'=>'Camion','city'=>'Citadine','motorcycle'=>'Moto',
                    ];
                ...
                @endphp
                <tr style="{{ $vehicle->trashed() ? 'opacity:.55;' : '' }}">
                    <td>
                        <div class="vehicle-cell">
                            ...
                            @else
                                <div class="vehicle-photo-placeholder">
                                    <svg width="18" height="14" fill="none" viewBox="0 0 24 18"><path d="M3 13h2l1-3h12l1 3h2" stroke="#94a3b8" stroke-width="1.8" stroke-linecap="round"/><circle cx="7.5" cy="14.5" r="1.5" stroke="#94a3b8" stroke-width="1.5"/><circle cx="16.5" cy="14.5" r="1.5" stroke="#94a3b8" stroke-width="1.5"/><path d="M6.5 5l1-3h9l1 3" stroke="#94a3b8" stroke-width="1.5" fill="none"/></svg>
                                </div>
                            @endif
                            <div>
                    ...
                    <td>
                        ...
                        @if($vehicle->trashed())<span class="badge" style="background:#fef2f2;color:#dc2626;margin-left:.3rem;">Archivé</span>@endif
                    </td>
                    <td>
                    ...
                    <td style="text-align:right;">
                        <div style="display:flex;justify-content:flex-end;gap:.4rem;">
                            ...
                            <form method="POST" action="{{ route('vehicles.restore', $vehicle->id) }}">
                                @csrf
                                <button type="submit" class="btn btn-ghost" style="padding:.35rem .7rem;color:#10b981;font-size:.78rem;" title="Restaurer">Restaurer</button>
                            </form>
                            @endcan
                            @endif
                        </div>
                ...
                @empty
                <tr><td colspan="7" style="text-align:center;padding:2.5rem;color:#94a3b8;">Aucun véhicule trouvé.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
