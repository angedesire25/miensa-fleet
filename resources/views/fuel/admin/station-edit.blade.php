...
@section('title', 'Modifier '.$fuelStation->name)
@section('page-title', 'Carburant')
@section('breadcrumb', 'Modifier station')
...
<div class="page-content" style="max-width:640px;">
    ...
    <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:1.5rem;">
        ...
        <h1 style="font-size:1.2rem;font-weight:700;color:#0f172a;margin:0;">Modifier : {{ $fuelStation->name }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('fuel.admin.station-update', $fuelStation) }}"
          style="background:#fff;border:1px solid #e2e8f0;border-radius:.75rem;padding:1.75rem;">
        @csrf @method('PUT')
        @include('fuel.admin._station-form', ['station' => $fuelStation])
        <div style="display:flex;gap:.75rem;justify-content:flex-end;margin-top:1.25rem;">
            ...
            <button type="submit"
                    style="padding:.6rem 1.4rem;background:linear-gradient(135deg,#3b82f6,#2563eb);color:#fff;border:none;border-radius:.45rem;font-size:.875rem;font-weight:600;cursor:pointer;">
                Mettre à jour
            </button>
        </div>
    </form>
